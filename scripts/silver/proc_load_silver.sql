...
BEGIN
	...
	BEGIN TRY
		...
		PRINT '-----------------------------------------------';
		PRINT 'Loading and Converting CRM Tables';
		PRINT '-----------------------------------------------';
		...
		PRINT '>>Inserting and Converting Data Into: silver.crm_prd_info';
		...
		PRINT '>>Inserting and Converting Data Into: silver.crm_cust_info';
		...
		SELECT
			...
			CASE WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
				 ...
				 ELSE 'n/a'
			END AS cst_gndr, -- Normalize gender data
			cst_create_date
		FROM (
			SELECT
				* ,
		...
		PRINT '>>Inserting and Converting Data Into: silver.crm_sales_details';
		...
		PRINT '-----------------------------------------------';
		PRINT 'Loading and Converting ERP Tables';
		PRINT '-----------------------------------------------';
		...
		PRINT '>>Inserting and Converting Data Into: silver.erp_cust_az12';
		...
		CASE WHEN bdate > GETDATE() THEN NULL
			ELSE bdate
		END AS bdate, -- Set Birthdays in the future as NULL
		...
		SELECT 
		REPLACE(cid, '-', '') cid, -- Remove '-' with empty string
		CASE 
			WHEN UPPER(TRIM(cntry)) IN ('USA', 'US') THEN 'United States'
			WHEN UPPER(TRIM(cntry)) IN ('DE') THEN 'Germany'
			WHEN UPPER(TRIM(cntry)) = '' OR cntry IS NULL THEN 'n/a'
			ELSE cntry
		END AS cntry -- Normalize Country data and handling missung values
